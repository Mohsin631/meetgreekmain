...
class CPage extends CHtmlBlock
{
	function parseBlock(&$html)
	{
        ...
        if ($page == 1){
            ...
            if ($pid > 0){
                $items = DB::all("SELECT id,title,user_id FROM gallery_albums WHERE `user_id` = ". to_sql($pid,"Number") ." LIMIT ". to_sql(($page - 1)*$pagerOnPage,"Number") . ",". to_sql($page * $pagerOnPage,"Number") . ";");
            } else {
                $items = DB::all("SELECT id,title,user_id FROM gallery_albums LIMIT ". to_sql(($page - 1)*$pagerOnPage,"Number") . ",". to_sql($page * $pagerOnPage,"Number") . ";");
            }
        }
