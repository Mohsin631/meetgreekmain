...
class CPlaces extends CHtmlList
{
	...
	function init()
	{
		...
		$this->m_sql_count = "SELECT COUNT(m.parent_comment_id) FROM groups_group_comment_comment AS m " . $this->m_sql_from_add . "";
		$this->m_sql = "
			SELECT m.*
	...
	function onItem(&$html, $row, $i, $last)
	{
        ...
        $this->m_field['parent_comment_id'][1] = DB::result("SELECT comment_text FROM groups_group_comment WHERE comment_id=" . $row['parent_comment_id'] . "", 0, 2);
