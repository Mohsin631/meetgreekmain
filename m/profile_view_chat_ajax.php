...
class CChatAjax extends CHtmlBlock
{
	...
	function onItem(&$html, $row)
	{
        ...
        if ($row['system']) {
            ...
            $msg = CIm::prepareMediaFromComment($row['msg'], $row['from_user']);
        }
        if ($isParse) {
            ...
            if($row['msg_translation']!=''  && Common::isOptionActive('autotranslator_show_original')){
                ...
                $msgTranslation = CIm::prepareMediaFromComment($row['msg_translation'], $row['from_user']);
                $html->setvar('msg_translation', $msgTranslation);
	...
	function parseBlock(&$html)
	{
            ...
            DB::query($this->m_sql . ' LIMIT ' . max(0, $num - $this->m_on_page) . ', ' . $this->m_on_page, 3);
