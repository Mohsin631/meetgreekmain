// Three.js - https://github.com/mrdoob/babylon.js/
// RoundedBoxGeometry - https://github.com/pailhead/babylon-rounded-box

var RubikAnimationEngine = function() {
    this.ids = [];
    this.animations = {};
...
RubikAnimationEngine.prototype = {
    constructor: RubikAnimationEngine,
    update: function() {
    ...
    add: function(animation) {
        ...
        animation.id = chat3d.world.location.uniqueID++;
...
var RubikAnimation = function(engine) {
    if (engine) this.start(engine);
};

RubikAnimation.prototype = {
    constructor: RubikAnimation,
    start: function() {
        var engine = chat3d.world.location.animEngine;
        engine.add(this);
    },
...
RubikWorld = function(game, engine) {
    RubikAnimation.call(engine);
    this.game = game;
    ...
    this.renderer = new BABYLON.WebGLRenderer({ antialias: true, alpha: true });
    this.renderer.setPixelRatio(window.devicePixelRatio);
    this.container.appendChild(this.renderer.domElement);

    this.camera = new BABYLON.PerspectiveCamera(2, 1, 0.1, 10000);
...
RubikWorld.prototype = Object.create(RubikAnimation.prototype);
RubikWorld.prototype.createLights = function() {

    this.lights = {
        holder: new BABYLON.Object3D,
        ambient: new BABYLON.AmbientLight(0xffffff, 0.69),
        front: new BABYLON.DirectionalLight(0xffffff, 0.36),
        back: new BABYLON.DirectionalLight(0xffffff, 0.19),
    };
    ...
    chat3d.world.scene.add(this.lights.holder);

};

RubikWorld.prototype.RemoveLights = function() {
    chat3d.world.scene.remove(this.lights.holder);
};



var RubikRoundedBoxGeometry = function(size, radius, radiusSegments) {

    BABYLON.BufferGeometry.call(this);
    ...
    var positions = new BABYLON.BufferAttribute(new Float32Array(totalVertexCount * 3), 3);
    var normals = new BABYLON.BufferAttribute(new Float32Array(totalVertexCount * 3), 3);

    var
        cornerVerts = [],
        cornerNormals = [],
        normal = new BABYLON.Vector3(),
        vertex = new BABYLON.Vector3(),
        vertexPool = [],
        normalPool = [],
        indices = [];
    ...
    function doVertices() {
        ...
        var cornerOffset = new BABYLON.Vector3(edgeHalfWidth, edgeHalfHeight, edgeHalfDepth);
    ...
    this.setIndex(new BABYLON.BufferAttribute(new Uint16Array(indices), 1));
    this.addAttribute('position', positions);
    this.addAttribute('normal', normals);

}

RubikRoundedBoxGeometry.prototype = Object.create(BABYLON.BufferGeometry.prototype);
RubikRoundedBoxGeometry.constructor = RubikRoundedBoxGeometry;


function RubikRoundedPlaneGeometry(size, radius, depth) {

    //console.log(size, radius, depth);
    ...
    //console.log(x, y, width, radius);

    const shape = new BABYLON.Shape();
    ...
    const geometry = new BABYLON.ExtrudeBufferGeometry(
        shape, { depth: depth, bevelEnabled: false, curveSegments: 3 }
...
RubikCube = function(game) {
    ...
    this.holder = new BABYLON.Object3D();
    this.object = new BABYLON.Object3D();
    this.animator = new BABYLON.Object3D();
...
RubikCube.prototype = {
        ...
        for (x = 0; x < this.size; x++) {
            for (y = 0; y < this.size; y++) {
                for (z = 0; z < this.size; z++) {
                    ...
                    let position = new BABYLON.Vector3(first + x, first + y, first + z);
                    let edges = [];
    ...
    generateModel: function() {
        ...
        const mainMaterial = new BABYLON.MeshLambertMaterial();

        const pieceMesh = new BABYLON.Mesh(
            new RubikRoundedBoxGeometry(pieceSize, this.geometry.pieceCornerRadius, 3 / 10),
            mainMaterial.clone()
        );

        const edgeGeometry = RubikRoundedPlaneGeometry(
            pieceSize,
            this.geometry.edgeCornerRoundness,
            this.geometry.edgeDepth
        );

        edgeGeometry.scale(1, 1, 0.0002);

        this.positions.forEach((position, index) => {

            const piece = new BABYLON.Object3D();
            ...
            position.edges.forEach(position => {
                ...
                const edge = new BABYLON.Mesh(edgeGeometry, mainMaterial.clone());
...
var RubikTween = function(options) {
    RubikAnimation.call();
    ...
    if (this.delay) setTimeout(() => this.start(), this.delay);
    else this.start();

    this.onUpdate(this);
};

RubikTween.prototype = Object.create(RubikAnimation.prototype);
RubikTween.prototype.constructor = RubikTween;

RubikTween.prototype.update = function(delta) {
...
RubikTween.prototype.updateYoyo = function() {
...
RubikTween.prototype.updateFromTo = function() {
...
RubikTween.prototype.getFromTo = function(options) {
...
var RubikControls = function(game) {
    this.game = game;
    ...
    this.raycaster = new BABYLON.Raycaster();

    const helperMaterial = new BABYLON.MeshBasicMaterial({ depthWrite: false, transparent: true, opacity: 0.5, color: 0x0033ff, side: 2, visible: false });

    this.group = new BABYLON.Object3D();
    this.group.name = 'controls';
    this.game.cube.object.add(this.group);

    this.helper = new BABYLON.Mesh(
        new BABYLON.PlaneBufferGeometry(2.5, 2.5),
    ...
    this.edges = new BABYLON.Mesh(
        new BABYLON.BoxBufferGeometry(1, 1, 1),
        helperMaterial.clone(),
    );
...
RubikControls.prototype = {

    initDraggable: function() {

        this.draggable = new RubikDraggable(this.game.dom.game);
        ...
        this.draggable.onDragStart = position => {
            ...
            //console.log(edgeIntersect, this.dragIntersect);
            ...
            if (edgeIntersect !== false && this.dragIntersect !== false) {
                ...
                this.helper.lookAt(chat3d.world.eyes.position);
                ...
                this.dragNormal = new BABYLON.Vector3(0, 0, 1);
                this.flipType = 'cube';
            ...
            //console.log(planeIntersect, position.current, this.helper);

            if (planeIntersect === false) return;

            this.dragCurrent = this.helper.worldToLocal(planeIntersect.point);
            this.dragTotal = new BABYLON.Vector3();
        ...
        this.draggable.onDragMove = position => {
            ...
            //console.log(this.state, this.gettingDrag);
            ...
            //console.log(this.state, this.dragTotal.length(), PREPARING, this.flipType);
            ...
            if (this.state === PREPARING && this.dragTotal.length() > 0.05) {
                ...
                if (this.flipType === 'layer') {
                    ...
                    this.flipAxis = new BABYLON.Vector3();
                ...
                if (this.flipType === 'layer') {
                    ...
                    this.edges.rotateOnAxis(this.flipAxis, rotation);
                    //console.log(this.edges.rotation);
    ...
    onMove: function() {
        if (this.scramble !== null) return;
        if (this.state === RybikSTILL || (this.state === RybikANIMATING && this.gettingDrag === false)) return;
        ...
        if (this.state === RubikPREPARING && this.dragTotal.length() > 0.05) {
            ...
            this.flipAngle = 0;
            this.state = RybikROTATING;

        } else if (this.state === RybikROTATING) {
    ...
    onEnd: function() {
        if (this.scramble !== null) return;
        if (this.state !== RybikROTATING) {

            this.gettingDrag = false;
            this.state = RybikSTILL;
            return;

        }

        this.state = RybikANIMATING;
        ...
        if (this.flipType === 'layer') {
            ...
            this.rotateLayer(delta, false, layer => {
                ...
                this.state = this.gettingDrag ? RybikPREPARING : RybikSTILL;
                this.gettingDrag = false;
    ...
    rotateLayer: function(rotation, scramble, callback) {
        ...
        this.rotationTween = new RubikTween({
            easing: easing,
            duration: duration,
            onUpdate: tween => {
    ...
    rotateCube: function(rotation, callback) {
        ...
        this.rotationTween = new RubikTween({
            ...
            onUpdate: tween => {
                ...
                this.edges.rotateOnAxis(this.flipAxis, tween.delta * rotation);
                this.game.cube.object.rotation.copy(this.edges.rotation);
    ...
    movePieces: function(layer, from, to) {
        //console.log(layer);
        ...
        layer.forEach(index => {
            ...
            from.remove(piece);
            piece.applyMatrix(new BABYLON.Matrix4().getInverse(to.matrixWorld));
            to.add(piece);
    ...
    keyboardMove: function(type, move, callback) {
    ...
    getIntersect: function(position, object, multiple) {
        ...
        //console.log(this.raycaster, this.draggable.convertPosition(position.clone()));
    ...
    attach: function(child, parent) {
        ...
        child.applyMatrix(new BABYLON.Matrix4().getInverse(parent.matrixWorld));
    ...
    addMomentumPoint: function(delta) {
    ...
    getMomentum: function() {
        ...
        const momentum = new BABYLON.Vector2();
...
var RubikScrambler = function(game) {
    this.game = game;
...
RubikScrambler.prototype = {
    constructor: RubikScrambler,
...
var RubikTransition = function(game) {
    this.game = game;
...
RubikTransition.prototype = {
    constructor: RubikTransition,
    init: function() {
    ...
    button: function(show, hide) {

    },


    cube: function(show, theming = false) {
        ...
        this.tweens.cube = new RubikTween({
    ...
    float: function() {
        ...
        this.tweens.float = new RubikTween({
            duration: 1500,
    ...
    zoom: function(play, time) {
        ...
        this.tweens.zoom = new RubikTween({
            target: this.game.world.camera,
            duration: duration,
            easing: easing,
            to: { zoom: zoom },
            onUpdate: () => { chat3d.world.camera.updateProjectionMatrix(); },
        });

        this.tweens.rotate = new RubikTween({
    ...
    ranges: function(ranges, type, show) {
    ...
    flipLetters: function(type, letters, show) {
...
RubikTimer.prototype = Object.create(RubikAnimation.prototype);

RubikTimer.prototype.constructor = RubikTimer;

RubikTimer.prototype.super_start = RubikTimer.prototype.start;

RubikTimer.prototype.start = function(continueGame) {
...
RubikTimer.prototype.reset = function() {
...
RubikTimer.prototype.super_stop = RubikTimer.prototype.stop;

RubikTimer.prototype.stop = function() {
...
RubikTimer.prototype.update = function() {
...
RubikTimer.prototype.convert = function() {
...
RubikTimer.prototype.setText = function() {
...
var RubikDraggable = function(element, options) {
    this.position = {
        current: new BABYLON.Vector2(),
        start: new BABYLON.Vector2(),
        delta: new BABYLON.Vector2(),
        old: new BABYLON.Vector2(),
        drag: new BABYLON.Vector2(),
    };
    ...
    this.drag = {
        ...
        start: (event) => {
            ...
            return this.onDragStart(this.position);
        ...
        move: (event) => {
            ...
            return this.onDragMove(this.position);
...
RubikDraggable.prototype = {
    constructor: RubikDraggable,
    enable: function() {
    ...
    getPositionCurrent: function(event) {
...
var RubikRange = function(name, options) {
...
RubikRange.prototype = {
    constructor: RubikRange,
    ...
    initDraggable: function() {
        ...
        this.draggable = new RubikDraggable(this.handle, { calcDelta: true });
    ...
    percentsFromValue: function(value) {

        return (value - this.min) / (this.max - this.min);

    },

    valueFromPosition: function(position) {
...
var RubikPreferences = function(game) {
    this.game = game;
};

RubikPreferences.prototype = {
    constructor: RubikPreferences,

    init: function() {

        this.ranges = {

            size: new RubikRange('size', {
            ...
            scramble: new RubikRange('scramble', {
            ...
            saturation: new RubikRange('saturation', {
                value: 100,
                range: [0, 100],
                onUpdate: value => this.game.themeEditor.updateHSL(),
                onComplete: () => this.game.storage.savePreferences(),
            ...
            lightness: new RubikRange('lightness', {
                value: 50,
                range: [0, 100],
                onUpdate: value => this.game.themeEditor.updateHSL(),
                onComplete: () => this.game.storage.savePreferences(),
...
var RubikConfetti = function(game) {
    this.game = game;
    this.started = 0;
    ...
    this.geometry = new BABYLON.PlaneGeometry(1, 1);
    this.material = new BABYLON.MeshLambertMaterial({ side: BABYLON.DoubleSide });

    this.holders = [
        new RubikConfettiStage(this.game, this, 1, 20),
        new RubikConfettiStage(this.game, this, -1, 30),
    ];
};

RubikConfetti.prototype = {
    constructor: RubikConfetti,
...
var RubikConfettiStage = function(game, parent, distance, count) {
    RubikAnimation.call(false);
    this.game = game;
    this.parent = parent;
    ...
    this.holder = new BABYLON.Object3D();
    this.holder.rotation.copy(chat3d.world.eyes.rotation);
    ...
    let i = this.count;
    while (i--) this.particles.push(new RubikParticle(this));
};

RubikConfettiStage.prototype = Object.create(RubikAnimation.prototype);

RubikConfettiStage.prototype.constructor = RubikConfettiStage;

RubikConfettiStage.prototype.super_start = RubikConfettiStage.prototype.start;

RubikConfettiStage.prototype.start = function() {
    this.time = performance.now();
    this.playing = true;
...
RubikConfettiStage.prototype.super_stop = RubikConfettiStage.prototype.stop;

RubikConfettiStage.prototype.stop = function(callback) {
...
RubikConfettiStage.prototype.reset = function() {
...
RubikConfettiStage.prototype.update = function() {
...
RubikConfettiStage.prototype.resizeViewport = function() {

    const fovRad = chat3d.world.camera.fov * BABYLON.Math.DEG2RAD;

    this.height = 2 * Math.tan(fovRad / 2) * (chat3d.world.eyes.position.length() - this.distanceFromCube);
    this.width = this.height * chat3d.world.camera.aspect;
...
RubikParticle = function(confetti) {
    this.confetti = confetti;
    this.options = this.confetti.options;

    this.velocity = new BABYLON.Vector3();
    this.force = new BABYLON.Vector3();

    this.mesh = new BABYLON.Mesh(this.confetti.geometry, this.confetti.material.clone());
    this.confetti.object.add(this.mesh);

    this.size = BABYLON.Math.randFloat(this.options.size.min, this.options.size.max);
    this.mesh.scale.set(this.size, this.size, this.size);

    return this;
};

RubikParticle.prototype = {
    reset: function(randomHeight = true) {

        this.completed = false;

        this.color = new BABYLON.Color(this.options.colors[Math.floor(Math.random() * this.options.colors.length)]);
        this.mesh.material.color.set(this.color);

        this.speed = BABYLON.Math.randFloat(this.options.speed.min, this.options.speed.max) * -1;
        this.mesh.position.x = BABYLON.Math.randFloat(-this.confetti.width / 2, this.confetti.width / 2);
        this.mesh.position.y = (randomHeight) ?
            BABYLON.Math.randFloat(this.size, this.confetti.height + this.size) :
            this.size;

        this.revolutionSpeed = BABYLON.Math.randFloat(this.options.revolution.min, this.options.revolution.max);
...
RubikScores.prototype = {
    constructor: RubikScores,
...
var RubikStorage = function(game) {
    this.game = game;
...
RubikStorage.prototype = {
    constructor: RubikStorage,
    init: function() {
    ...
    saveGame: function() {
        ...
        chat3d.world.location.hub.broadcast('gameState', gameCubeData);
...
RubikThemes.prototype = {
    constructor: RubikThemes,
    getColors: function() {
        return this.colors[this.theme];
    },

    setTheme: function(theme = false, force = false) {
...
var RubikThemeEditor = function(game) {
    this.game = game;
...
RubikThemeEditor.prototype = {
    constructor: RubikThemeEditor,
    colorFromHSL: function(h, s, l) {
        ...
        return new BABYLON.Color(`hsl(${h}, ${s}%, ${l}%)`);

    },

    setHSL: function(color = null, animate = false) {

        this.editColor = (color === null) ? 'R' : color;

        const hsl = new BABYLON.Color(this.game.themes.getColors()[this.editColor]);
    ...
    getPieceColor: function(event) {

        const clickEvent = event.touches ?
            (event.touches[0] || event.changedTouches[0]) :
            event;

        const clickPosition = new BABYLON.Vector2(clickEvent.pageX, clickEvent.pageY);
...
var RubikIconsConverter = function(options) {
    options = Object.assign({
        tagName: 'icon',
        className: 'icon',
        styles: false,
...
RubikIconsConverter.prototype = {
    constructor: RubikIconsConverter,

    convertAllIcons: function() {
...
var RubikIcons = new RubikIconsConverter({
...
var RubikGame = function() {
    ...
    this.world = new RubikWorld(this);
    this.cube = new RubikCube(this);
    this.controls = new RubikControls(this);
    this.scrambler = new RubikScrambler(this);
    this.transition = new RubikTransition(this);
    this.timer = new RubikTimer(this);
    this.preferences = new RubikPreferences(this);
    this.scores = new RubikScores(this);
    this.storage = new RubikStorage(this);
    this.confetti = new RubikConfetti(this);
    this.themes = new RubikThemes(this);
    this.themeEditor = new RubikThemeEditor(this);

    this.initActions();

    this.state = RubikSTATE.Menu;
    this.newGame = false;
    this.saved = false;
    ...
    setTimeout(() => {
        ...
        this.transition.cube(RubikSHOW);

        /*setTimeout(() => this.transition.title(RubikSHOW), 700);*/
        /*setTimeout(() => this.transition.buttons(RubikBUTTONS.Menu, RubikBUTTONS.None), 1000);*/
...
RubikGame.prototype = {
    ...
    game: function(show) {
        if (show) {
            ...
            this.state = RubikSTATE.Playing;
            //this.saved = true;

            //this.transition.buttons(RubikBUTTONS.None, RubikBUTTONS.Menu);

            this.transition.zoom(RubikSTATE.Playing, duration);
            //this.transition.title(RubikHIDE);

            setTimeout(() => {

                this.transition.timer(RubikSHOW);
                //this.transition.buttons(RubikBUTTONS.Playing, RubikBUTTONS.None);
    ...
    theme: function(show) {
        ...
        if (show) {
            ...
            this.cube.loadFromData(RubikStates['3']['checkerboard']);
    ...
    complete: function(show) {
        ...
        if (show) {
            ...
            this.transition.buttons(RubikBUTTONS.Complete, RubikBUTTONS.Playing);

            this.state = RubikSTATE.Complete;
            this.saved = false;
            ...
            this.transition.zoom(RubikSTATE.Menu, 0);
            this.transition.elevate(RubikSHOW);

            setTimeout(() => {

                this.transition.complete(RubikSHOW, this.bestTime);
                this.confetti.start();
            ...
            this.state = RubikSTATE.Stats;
            this.saved = false;

            this.transition.timer(RubikHIDE);
            this.transition.complete(RubikHIDE, this.bestTime);
            this.transition.cube(RubikHIDE);
            this.timer.reset();
            ...
            setTimeout(() => {
                ...
                this.transition.stats(RubikSHOW);
...
locations[51].onInit = function() {
    this.uniqueID = 0;
    this.animEngine = new RubikAnimationEngine();
    this.game = new RubikGame();
    chat3d.world.scene.add(this.game.world.scene);
    this.game.world.scene.scale.set(250, 250, 250);

    //console.log(this.game.world.scene);
    this.game.world.scene.position.y = 40;
    this.game.game(true);

    this.resetbox = new BABYLON.Mesh(new BABYLON.CubeGeometry(20, 4, 20), new BABYLON.MeshBasicMaterial({
        visible: false
    }));
    this.resetbox.position.set(0, 90, -610);
    this.resetbox.rotation.x = Math.PI / 4;
    chat3d.world.scene.add(this.resetbox);

    this.hub = new p2phub();

    this.isResetable = function(x, y) {
    ...
    this.setGameState = function(data, age, counter) {
        ...
        this.game.cube.loadFromData(data);
    };



    this.set_player = function(id) {

        if (id != this.player) {

            if (this.player == chat3d.selfid && id != chat3d.selfid) {
...
locations[51].onMessage = function(data) {
    if (data) {
...
locations[51].onClean = function() {
    chat3d.world.scene.remove(this.game.world.scene);
    chat3d.world.scene.remove(this.resetbox);
    $('canvas').removeClass('pointer_on_game_obj');
    $('canvas').removeClass('pointer_on_game_obj_move');
};

locations[51].onRender = function(delta) {
    this.animEngine.update();
    //console.log(this.game.world.scene.children[0].position);
    this.uniforms.pos.value.copy(this.game.world.scene.children[0].position).multiplyScalar(1000);
    this.uniforms.pos.value.y -= 150;
};



locations[51].onView = function(x, y) {
    //console.log(this.game.controls.helper);
    var raycast = chat3d.view.raycaster;
    raycast.ray.origin.setFromMatrixPosition(chat3d.world.camera.matrixWorld);
    ...
    raycast.far = 1000;
    raycast.ray.direction = vector;
    intersects = raycast.intersectObject(this.game.controls.helper);
    if (intersects.length) {
        $('canvas').removeClass('pointer_on_game_obj');
...
locations[51].onMouseMove = function(e) {
    this.game.controls.draggable.drag.move(e);
    return true;
};

locations[51].onMouseDown = function(e) {
    if (!this.player || this.player == chat3d.selfid) {
        if (this.game.controls.draggable.drag.start(e)) {
            this.set_player(chat3d.selfid);
            this.hub.broadcast('set_player', chat3d.selfid);
            return false;
        }
        this.resetBut(e.clientX, e.clientY);
    }
    return true;
};

locations[51].onMouseUp = function(e) {
    if (!this.player || this.player == chat3d.selfid) {
        this.game.controls.draggable.drag.end(e);
    }
    return true;
};


locations[51].onTouchUpEnd = function(e) {
    if (!this.player || this.player == chat3d.selfid) {
        this.game.controls.draggable.drag.end(e);
    }
    return true;
...
locations[51].onTouchDown = function(e) {
    if (!this.player || this.player == chat3d.selfid) {
        if (this.game.controls.draggable.drag.start(e)) {
            this.set_player(chat3d.selfid);
            this.hub.broadcast('set_player', chat3d.selfid);
        ...
        this.resetBut(e.changedTouches[0].clientX, e.changedTouches[0].clientY);
    }
    return true;
};

locations[51].onTouchMove = function(e) {
    if (!this.player || this.player == chat3d.selfid) {
        if (this.game.controls.draggable.drag.move(e)) {
            return false;
        }
    }
    return true;
...
locations[51].onLoop = function(delta) {
    if (this.player) {
        this.player_ttl -= delta;
        if (this.player_ttl < 0) {
            if (this.player == chat3d.selfid) {
                this.hub.broadcast('set_player', null);
            }
            this.player = null;
        }
    }
