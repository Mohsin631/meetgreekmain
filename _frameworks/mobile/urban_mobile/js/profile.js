var CProfile = function(guid, is_free_site, is_credits_enabled) {
    ...
    this.updateServerMyData = function(allowedFeature){
        ...
        if ($('.pp_message_upload_img_bl:visible')[0]) {
    ...
    this.giftsList=[];
    this.initGiftsList = function(){
        $this.giftsList=$('.bl_gift ul');
        var fnResize=function(){
            if(!$this.giftsList[0])return;
            var h=$this.isCreditsEnabled?270:238;
            $this.giftsList.height($win.height()-h);
        }

        fnResize();
        $win.on('resize orientationchange',fnResize);
    }
    ...
    this.openPopupEditorVerification = function(){
        ...
        showConfirm(msg, l('verify'), $this.langParts.cancel, function($btn){
            ...
            return false;
        },'blue','#st-container',false,true,'pp_verification');
    ...
    $(function(){
        ...
        $this.$userMenu = $('.item', '#menu-3').on('click',function(e){
            ...
            if (this.id=='user_menu_verify_account') {
                $this.openPopupEditorVerification();
                return false;
            }
            $this.clearPageBeforeRedirect();
