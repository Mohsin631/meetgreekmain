var CMessages = function(guid, imHistoryMessages) {
    ...
    this.send = function(msg){
        ...
        if(msg!==''||isUploadImage){
            if(!$this.isAjax['send'])$this.$btnSend.html(getLoaderCl('loader_send'));
            $this.isAjax['send']++;
            msg=emojiToHtml(msg);
            var data={cmd:'send_message',display:'one_chat',ajax:1,user_to:userTo,msg:msg,to_delete:0};
            if (isUploadImage) {
                data['image_upload']=$this.isImageLoaded?1:0;
                data['ind']=$this.isImageLoaded;
            }
            $.post(url_main+'messages.php',data,
                function(res){
                    var data=checkDataAjax(res);
                    if(data!==false){
    ...
    this.prepareStyle = function(){
        var w=$win.width()-(isOneChat=='general_chat'?102:140);
    ...
    this.checkPaydUploadImage = function() {
        if(!userAllowedFeature['upload_image_chat_paid']){
            showConfirm($this.langParts.upload_im_need_upgrade, $this.langParts.ok, $this.langParts.cancel, function(){
    ...
    this.initAddImage = function(){
        ...
        $('#pp_message_upload_img_frm').submit(function(e){
            var indx=+new Date,
                ...
                if (error) {
                    showAlert(error,'#st-container');
                    return false;
                }
                $this.processUploadImage=true;
                $this.addImageIm.addClass('to_hide').attr('title', '');
                var $loader=getLoaderCl('loader_send').appendTo($this.addImageIm);
