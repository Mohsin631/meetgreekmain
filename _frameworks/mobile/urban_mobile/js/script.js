...
function showConfirm(msg, btnDone, btnCancel, callback, vis, wr, callbackCancel,noHiddenDone,clTip){
    noHiddenDone=noHiddenDone||false;
    clTip=clTip||'';
    var html,vis=vis||'red',wr=wr||'#st-container',
        tip=$('.tip_confirm'),$shadow=$('#page_shadow_empty');
    if(!tip[0]){
        html = '<div class="bl_tooltip_btn center tip_confirm '+clTip+'" >'+
    ...
    $('a.btn_done', tip).css('width','auto')
    .html('<span>'+btnDone+'</span>')
    .data('clLoader','btn_action_loader_alert')
    .one('click',function(){
        if(typeof callback==='function'){
            callback($(this));
        }
        if(!noHiddenDone)hideConfirm();
        return false;
    });
...
function prepareInterests() {
    ...
    if (i[0]) {
        var last=i.last(), hi=i.eq(0).outerHeight();
        var $ulGift=$('#profile_gifts_list').find('ul'), h=0;
        if($ulGift[0]){
            h=$ulGift.outerHeight()+15;
        }
