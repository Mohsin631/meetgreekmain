var CMessages = function(guid) {
                    ...
                    '<div class="item">'+
                        '<div class="msg">'+
                            ...
                            '<div class="info">'+
                                ...
                                '<div class="msg_txt"><span class="blink_msg">...</span></div>'+
                            '</div>'+
                        '</div>'+
                    '</div>'+
                '</div>');
    ...
    this.send = function(msg){
        ...
        if($this.isImageLoaded){
            $msg.find('.msg_txt').html('<div class="mod_image_preload">...</div>' + '\n');
        }

        var $node=$msg.find('.msg_txt');
        $node[0].innerHTML +=strToHtml(msg);
    ...
    this.initLightbox = function($img) {
        if(!$img[0])return;
        var fn=function(){
        $this.maxWidthLightbox=$win.width();
        var data={
            resizeImage:   true,
            deltaWidth : 20,
            deltaWrap: 20,
    ...
    this.prepareStyle = function(){
        var w=$win.width()-140;
        $('#message_custom_style')[0].innerHTML=[
                "#lightbox-wrap{height:", $win.height(), "px;}",
                ".mod_im_msg_video .one_media_youtube{width:", w, "px; height:", Math.round(w/1.247), "px;}"
                ].join("");
    }

    this.checkPaydUploadImage = function() {
        if(!userAllowedFeature['upload_image_chat_paid']){
            showConfirm(l('upload_im_need_upgrade'),
            function(){
                uploadPageUpgrade([],'',true)
            })
            return true;
        }
        return false;
    ...
    this.clearUploadImage = function(reset,checkPayd){
        ...
        $('#pp_message_upload_img').attr('title', l('upload_image'));
    ...
    this.initAddImage = function(){
        ...
        $('#pp_message_upload_img_frm').submit(function(e){
            var indx=+new Date,
                ...
                if (error) {
                    showAlert(error,false,'fa-info-circle')
                    return false;
                }
                $this.processUploadImage=true;
                $this.addImageImLoader.removeClass('add_loader').addChildrenLoader();
                        ...
                        if(xhr.status == 200) {
                            ...
                            if (data) {
                                if (data!='complete_upload_image') {
                                    showAlert(data,false,'fa-info-circle');
                                } else {
                                    $this.isImageLoaded=indx;

                                    $('#field_send').removeClass('disabled');
                                    $('#pp_message_upload_img').attr('title', l('upload_image_delete'));
                                }
                            }
                            $this.processUploadImage = false;
                            $this.addImageIm.removeClass('to_hide');
