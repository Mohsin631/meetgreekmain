...
class IpInfo {
   ...
   * @var string
   */
  const FIELD_NOT_SUPPORTED = 'This parameter is unavailable in selected data file.';
  ...
  private function readWord($pos) {
    ...
    $result = unpack('V', $this->read($pos - 1, 4));
    return self::wrap32($result[1]);
  }
  ...
  private function readByte($pos) {
    ...
    $result = unpack('C', $this->read($pos - 1, 1));
    return self::wrap8($result[1]);
  }
