...
class Gallery
{
    ...
    static function uploadIm($descImage)
	{
        global $g, $g_user;

        $file = self::getTempFileUploadImageIm();
        if (file_exists($file)) {
             return self::upload($file, 'chat_system', 'Chat', $descImage, '', true);
       }
        return 0;
    }

    static function getTempFileUploadImageIm()
    {
        ...
        return Common::getOption('dir_files', 'path') . 'temp/tmp_chat_' . guid() . $ind . '.jpg';
     }
    ...
    static function uploadImage($file, $folder)
	{
        ...
        if ($folder == 'chat_system') {//Upload image Im
            $sizesBaseImage['b']['w'] = 1920;
            $sizesBaseImage['b']['h'] = 1080;
        }
