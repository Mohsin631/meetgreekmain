...
class Groups {
    ...
    static function subscribeRequestAdd($uid, $groupId, $accepted = false, $isSendMail = true) {
        ...
        if ($isSendMail && Common::isEnabledAutoMail('group_subscribe_request')
            ...
            Common::sendAutomail($userInfo['lang'], $userInfo['mail'], $typeMail, $vars);
        }
    }
    ...
    static function getListSubscribers($groupId = null, $online = false, $limit = '', $fidIndex = false) {
        global $g;
    ...
    static function getNumberSubscribers($groupId = null) {
    ...
    static function sendReport($groupId = null) {
        ...
        $where = '`group_id` = ' . to_sql($groupId);
        $usersToReport = DB::field(self::$table, 'users_reports', $where);
        ...
        if ($usersToReport != $updateUsersToReport) {
            ...
            DB::update(self::$table, array('users_reports' => $updateUsersToReport), $where);
        }
    ...
    static function isReport($groupId, $uid = null, $groupInfo = null) {
        ...
        $whereReport = '`user_from` = ' . to_sql($uid)
                     . ' AND `group_id` = ' . to_sql($groupId) . ' AND `photo_id` = 0';
    ...
    static function delete($groupId, $admin = false) {
        ...
        $sql = 'SELECT id, photo_id FROM photo_comments WHERE user_id = ' . to_sql($user_id);
