        ...
        if (is_array($this->settings)) {
            ...
            foreach($this->settings as $key => $item) {
                ...
                if (isset($item['active']) && ($item['active'] == "Y" || $item['active'] == true)) {
                    ...
                    if (isset($item['group'])) {
                        if (isset($parseGroups[$item['group']])) {
                            $parseGroups[$item['group']]++;
                        } else {
                            $parseGroups[$item['group']] = 1;
                        }
                    }
                }
            ...
            if (!isset($parseGroups[1])
                 && (!Common::isOptionActive('free_site') || Common::isOptionActive('user_choose_time_zone'))) {
                $parseGroups[1] = 1;
            }
            foreach ($parseGroups as $group => $count) {
                $html->parse('fields_group_' . $group, false);
            }
    ...
    public function fieldsList() {
        ...
        if ($zones) {// && !Common::isMobile()
                $this->settings['timezone'] = array(
                    ...
                    'type' => 'select',
                    'active' => Common::isOptionActive('user_choose_time_zone'),
                    'group' => -1
                );
        }
