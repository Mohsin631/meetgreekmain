    ...
    static public function filter_to_html($text, $start_tag, $end_tag, $a_class, $target = '_blank', $isCalcMaxWidth = false, $prfId = '', $imMsg = false, $parseMetaLink = false)
    {
        ...
        foreach ($ids as $id){
        	...
        	if ($image){
                ...
                if ($tmplImageWidth && self::$addStyleLoad !== false) {
                    ...
                    if (!$newWidth || !$newHeight) {
                        if (file_exists($image_th)) {
                            $imageSize = @getimagesize($image_th);
                ...
                if ($isEdgeWall && !$imMsg) {
                    $tagHtml = $start_tag
                                . '<a data-id="' . $tagId . '"' . $targetBlank . ' class="' . $a_class . ' ' . $tagId . '" href="' . $image_b . '">'
                    ...
                    if ($sitePart != 'administration') {
                        $tagHtml .= "<script>onLoadImgTimeLine('" . $tagId . "');</script>";
                        //$tagHtml .= "<script>clWall.onLoadImgTimeLine('" . $tagId . "');</script>";
                    }
                } else {
                    $tagHtml = $start_tag
                                    . '<a target="' . $target . '" class="' . $a_class . '" href="' . $image_b . '">'
