...
class CVidsTools
{
	...
	static public function countUsers($isGroups = false)
	{
        $where = 'vid_videos > 0';
        if ($isGroups) {
            $where = 'vid_videos_pages > 0 OR vid_videos_groups >0';
        }
        return DB::count('user', $where);
    }

	static public function getUsers($limit = '0,6', $isGroups = false)
	{

        $whereGroup = 'group_id = 0';
        $where = 'vid_videos > 0';
        $order = 'vid_visits DESC, vid_comments DESC, vid_videos DESC';
        if ($isGroups) {
            $where = 'vid_videos_pages > 0 OR vid_videos_groups >0';
            $order = 'vid_videos_pages DESC, vid_videos_groups DESC, vid_comments DESC, vid_videos DESC';
            $whereGroup = 'group_id > 0';
        }

        $pops = DB::select('user', $where, $order, $limit);
        foreach ($pops as $k => &$v) {
            $v['photo'] = urphoto($v['user_id']);
            if ($isGroups) {
                $v['vid_videos'] = $v['vid_videos_pages'] + $v['vid_videos_groups'];
            }
            $whereSql = $whereGroup . ' AND `user_id` =' . to_sql($v['user_id']);
            $v['vid_comments'] = DB::count('vids_comment', $whereSql);

            $sql = 'SELECT SUM(count_views) AS count FROM vids_video WHERE ' . $whereSql;
	...
	static public function countVideosAdmin($where = '')
	{
        return self::_countVideosFilter($where);
    }
	static public function getVideosAdmin($where = '', $limit = '0,2')
	{
        $items = self::_getVideos($where, 'dt DESC', $limit);
        foreach ($items as $key => $item) {
            if ($item['group_id']) {
                $items[$key]['group_id'] = $item['group_id'];
                $items[$key]['user_name'] = Groups::getInfoBasic($item['group_id'], 'title');
            }
        }
        return $items;
    }
    ...
    /* Selections by user */
	static public function countVideosByUser($user_id, $where = '')
	{
        return self::_countVideos("user_id='" . intval($user_id) . "'" . $where);
    }
	static public function getVideosByUser($user_id, $limit = '0,10', $where = '')
	{
        return self::_getVideos("user_id='" . intval($user_id) . "'" . $where, 'dt DESC', $limit);
    }
    ...
    {
        return DB::count('vids_video', self::addWherePrivateCondition(), 'RAND()');
    }
	...
	static public function getVideosByQuery($q, $limit = '0,10')
	{
        ...
        $videos = DB::select('vids_video', $where, 'dt DESC', $limit);
        $videos_r = array();
    ...
    /* Comments to video */
	static public function countCommentsNew($where = '')
	{
        return DB::count('vids_comment', $where);
    }
	static public function getCommentsNew($where = '', $limit = '0,20')
	{
        $rows = DB::select('vids_comment', $where, 'dt DESC', $limit);
        $rows_r = array();
        foreach ($rows as $k => $v) {
            ...
            if ($v['group_id']) {
                ...
                $data['user_name'] = Groups::getInfoBasic($v['group_id'], 'title');
            }
            $rows_r[] = $data;
        }
        return $rows_r;
	...
	static public function delCommentByIdByAdmin($id)
	{
        ...
        self::deleteCommentVideoByAjax($id);
	...
	static public function insertVideo($file="", $active=1, $isAddData = true, $isUploaded = 0, $notUploadFile = false)
	{
        ...
        if (self::validateVideoCode() && !$notUploadFile) {
...
class VideoUploadService
{
    ...
    static function convert($video_file, $output_video, $output_img, $format='mp4')
    {
        ...
        if (isset($ca['flv']) and $ca['flv'] and isset($ca['jpeg']) and $ca['jpeg'] and isset($ca['path']) and $ca['path']) {
...
class VideoUploadFfmpeg
{
    ...
    static function convert($video_file, $output_video, $output_img)
    {
        ...
        $cmd = $ffmpeg . ' -i ' . $video_file . $strictExperimental . ' -movflags +faststart -crf 29 -vf "scale=min(iw*720/ih\,1280):min(720\,ih*1280/iw), pad=1280:720:(1280-iw)/2:(720-ih)/2, setsar=1" ' . $output_video;
        exec($cmd);
        //exec($flvtool2 . ' -UkP ' . $output_video);
        if (!file_exists($output_video) || !filesize($output_video)) {
            @unlink($output_video);
            return false;
        }

        if ($output_img) {
            $cmd = $ffmpeg . ' -i ' . $video_file . $strictExperimental . ' -qscale 1 -vframes 1 -s 1280x720 -ss 1.2 -vf "scale=min(iw*720/ih\,1280):min(720\,ih*1280/iw), pad=1280:720:(1280-iw)/2:(720-ih)/2, setsar=1" -f image2 ' . $output_img;
            //echo $cmd;
            exec($cmd);
        }

        if(!file_exists($output_img) || !filesize($output_img)) {
            @unlink($output_img);
            return false;
        }
    ...
    static function concatChunks($filePart, $filePartTemp, $chunkNumber)
    {
        global $g;
        ...
        $chunk1 = '';
        $chunk2 = '';
        $chunkFileTemp =  $g['path']['dir_files'] . "video/{$filePartTemp}";
        $chunkFileTempMp4 =  "{$chunkFileTemp}.mp4";
        for ($i = 0; $i <= $chunkNumber; $i++) {
            if (!$chunk1) {
                $chunk1 = $g['path']['dir_files'] . "temp/{$filePart}_{$i}";
                $chunk1Start = $chunk1;
            } else {
                $chunk2 = $g['path']['dir_files'] . "temp/{$filePart}_{$i}";
                $result = self::concat($chunk1, $chunk2, $chunkFileTempMp4);
                $chunk1 = $chunkFileTemp;
            }
        }

        if ($chunk1Start) {
            unlink($chunk1Start . '.mp4');
        }

        return $chunkFileTempMp4;

    }

    static function concat($chunk1, $chunk2, $chunkFileTemp)
    {
        global $g;

        $chunkFile1 = "{$chunk1}.mp4";
        $chunkFileTs1 = "{$chunk1}.ts";

        $chunkFile2 = "{$chunk2}.mp4";
        $chunkFileTs2 = "{$chunk2}.ts";
        ...
        $cmd =  $ffmpeg . ' -i ' . $chunkFile1 . $strictExperimental . ' -c copy -bsf h264_mp4toannexb ' . $chunkFileTs1;
        exec($cmd);

        $cmd =  $ffmpeg . ' -i ' . $chunkFile2 . $strictExperimental . ' -c copy -bsf h264_mp4toannexb ' . $chunkFileTs2;
        exec($cmd);

        $cmd =  $ffmpeg . ' -y -i "concat:' . $chunkFileTs1 . '|' . $chunkFileTs2 . '|" -c copy -bsf:a aac_adtstoasc ' . $chunkFileTemp;
        exec($cmd);
...
class VideoUpload
{
    public static $converterClass = null;
    public static $error = '';

    static function getService()
    {
        if(self::$converterClass === null) {
            if (file_exists(Common::getOption('dir_include', 'path') . 'config/ffmpeg.txt')) {
                self::$converterClass = 'VideoUploadFfmpeg';
            } else {
                self::$converterClass = 'VideoUploadService';
            }
        }

        return self::$converterClass;
    }

    static function getPathToFile($file, $folder = '')
    {
        if(self::$converterClass === 'VideoUploadService') {
            $pathToFile = self::urlFilesVideo($folder) . $file . '.txt';
        } else {
            $pathToFile = Common::getOption('dir_files', 'path') . "{$folder}/{$file}.txt";
        }

        return $pathToFile;
    }

    static function upload($id, $file, $format = 'mp4', $folder = 'video', $outputImage = true)
    {
            ...
            if($id==0)
            {
                $serviceClass = self::getService();

                // convert file
                // host path to file
                $file_id = $g['path']['dir_files'] . "{$folder}/" . $file;

                DB::close();

                $imageFile = $file_id . '_.jpg';
                if (!$outputImage) {
                    $imageFile = '';
                }

                $result = $serviceClass::convert(self::getPathToFile($file, $folder), $file_id . '.' . $format, $imageFile, $format);
    ...
    static function delete($id)
    {
            ...
            $path = array($file . '.txt', $file . '.mp4', $file . '.flv', $file . '.jpg',
                          $file . '_b.jpg', $file . '_bm.jpg', $file . '_src.jpg');
            ...
            /* Live */
            if (file_exists($file . '_bm.jpg')) unlink($file . '_bm.jpg');
            $fileTmp = $g['path']['dir_files'] . 'temp/live_image_' . $id . '_bm.jpg';
            if (file_exists($fileTmp)) unlink($fileTmp);
            /* Live */
    }
    static function urlFilesVideo($folder = 'video')
    {
        global $g;

        $url = Common::urlSiteSubfolders() . $g['dir_files'] . $folder . '/';
    ...
    static function downloadReadyFiles($id, $urlVideo, $fileSize)
    {
        global $g;
        ...
        if($id && $urlVideo) {
            ...
            $path = array($basePath . '.mp4');
            //$path = array($basePath . '.mp4', $basePath . '_src.jpg');
            //$imageSrcPath = $basePath . '_src.jpg';

            if(copyUrlToFile($urlVideo, $path[0])) {

                // wait for end of file write to disk
                sleep(3);
                $savedFileSize = bigFileSize($path[0]);
                ...
                if($savedFileSize == $fileSize) {
                    ...
                    self::$error = "Saved file size is incorrect $fileSize:$savedFileSize";
                    @unlink($path[0]);
                }
