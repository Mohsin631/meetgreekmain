...
class CBanner extends CHtmlBlock
{
    ...
    static public function getBlock(&$html, $type, $prf = '')
	{
        ...
        $isCheckAdmobVisitor = false;
		if ($isCheckAdmobVisitor && !guid() && strpos($type, 'admob') !== false) {
			$pageNoAdmobTmpl = array('edge' => array('index.php', 'join.php', 'about.php', 'info.php'));
			if (isset($pageNoAdmobTmpl[$optionNameTmpl])) {
				if (in_array($p, $pageNoAdmobTmpl[$optionNameTmpl])) {
					return false;
				}
			} else {
				return false;
